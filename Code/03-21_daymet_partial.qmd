...
```{r}
write_csv(daymet_all_unnest, "../data/fieldweather1.csv")
```

## For all site for all year 
```{r, for all site}
daymet_all_site <- field %>% 
  #slice(1:5) %>% 
  mutate(weather = pmap(list(.y = year,
                             .site = site,
                             .lat = lat,
...
```{r unnest weather for all year & site}

daymet_all_site_unnest <- daymet_all_site %>% 
  unnest(weather) %>% 
           pivot_wider(names_from = measurement,
                       values_from = value ) %>% 
  janitor::clean_names()

daymet_all_site_unnest
...
```{r}
write_csv(daymet_all_site_unnest, "../data/Allfieldweather.csv")
```
# Summary  
In this exercise, we:  
